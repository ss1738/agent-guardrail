...
- Package: `qedra` 0.2.0, Python 3.9+, `pip install qedra`.
- Runs as a stdio MCP server. Client config:
  ```json
  "qedra": {
    "command": "python3",
    "args": ["/path/to/qedra/integrations/mcp_server.py"],
...
**DONE.** The MCP server now lives in the package at `qedra/mcp_server.py` with a `main()`, wired as a console script in `pyproject.toml`:
```toml
qedra-mcp = "qedra.mcp_server:main"
```
So after `pip install 'qedra[mcp]'` the server is just `qedra-mcp`. The old `integrations/mcp_server.py` path still works (backward-compatible shim).

**Remaining:** publish `qedra` to PyPI so the registries can install it:
```bash
...
```
- [ss1738/qedra](https://github.com/ss1738/qedra) 🐍 🏠 - A runtime gate for coding agents: gates every run_shell / write_file / git call and blocks the ones that wreck a repo (force-push main, rm -rf, secret exfil, CI wipe). Git-branch policy machine-checked by z3.
```
...
```json
{
  ...
  "repository": { "url": "https://github.com/ss1738/qedra", "source": "github" },
  "version": "0.2.0",
  "packages": [
    {
      "registryType": "pypi",
