...
class Executor:
    def __init__(self, sandbox: str, guard: Guardrail):
    ...
    def git(self, args: str) -> str:
        ...
        d = self.guard.check(Action("git", op=op, branch=branch, force=force, hard=hard))
        if d.verdict != "ALLOW":
    ...
    def write_file(self, path: str, content: str) -> str:
        d = self.guard.check(Action("write", path=path, content=content))
        if d.verdict != "ALLOW":
    ...
    def run_shell(self, cmd: str) -> str:
        d = self.guard.check(Action("shell", cmd=cmd))
        if d.verdict != "ALLOW":
